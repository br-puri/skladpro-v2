...
This is a single-file Flask application (`app.py`, ~4 000 lines) backed by PostgreSQL (Neon). There are no separate modules — all routes, DB helpers, and PDF generation live in `app.py`.
...
- `CHROME_PATH`: must be `/usr/bin/chromium` in container; defaults to macOS Chrome path for local dev
